...
Relying Party Registration is addressed in Article 5b (11) of the Regulation and the [CIR for Relying Party Registration] lays down further details, which are in scope of present technical document: The registration of a Relying Party to a registry shall include at least the information set out in Annex I of the [CIR for Relying Party Registration] (see Art. 3(2)). This specification includes additional information (of technical and operational relevance) not mentioned explicitely in the Regulation as it allows for differentiating between a Wallet-Relying Party (a legal entity) and its registered services ('Wallet-Relying Party Service').
...
The Description row in the table contains clarifications regarding use of mandatory attributes with various Wallet-Relying Party Service types (entitlements and attestations provided  (named as *sub-entitlements* in the [CIR for Relying Party Registration]).
...
| Service Identifier   |    Identifier of the service the Wallet-Relying Party provides.   |  WalletRelyingParty.WalletRelyingPartyService.serviceIdentifier      |
| Service Trade Name   |   A user-friendly name (trade name or service name) of the service the wallet-relying party is registering.   |  WalletRelyingParty.WalletRelyingPartyService.serviceTradeName     |
|  Contact Information    | a Wallet-relying Party SHALL provide detailed contact information pertaining to its service registration and intended use of the wallet units in form of **at least one of** (a) a website for helpdesk and support, (b) a phone number or (c) an e-mail address.    |   WalletRelyingParty.WalletRelyingPartyService.supportURI  WalletRelyingParty.WalletRelyingPartyService.phone  WalletRelyingParty.WalletRelyingPartyService.email   |
| Service Description   |    Localised descriptions of the service the wallet-relying party provides.   |  WalletRelyingParty.WalletRelyingPartyService.srvDescription      |
| Entitlements  |  The entitlement(s) of the Wallet-Relying Party Service. Possible entitlement types set by the [CIR for Relying Party Registration] are 'Service_Provider', 'QEAA_Provider', 'Non_Q_EAA_Provider', 'PUB_EAA_Provider', 'PID_Provider', 'QCert_for_ESeal_Provider', 'QCert_for_ESig_Provider', 'rQSigCDs_Provider', 'rQSealCDs_Provider' or 'ESig_ESeal_Creation_Provider'. The [ETSI TS 119 475] defines a normative URI for each entitlement type that SHALL be used to indicate the registered entitlement.  | WalletRelyingParty.WalletRelyingPartyService.entitlement |
| Purpose | A description of intended use of the data that the Wallet-Relying Party Service intends to request from wallet units. **Provided for each intended use of the Wallet-Relying Party.**    |    WalletRelyingParty.WalletRelyingPartyService.IntendedUse.purpose       |
| Data Requested    | Provided the Wallet-Relying Party Service intends to request data from the Wallet Unit for its intended use, the attestations and attributes it intends to request. Provided from the registry in a machine readable format that provides the attestation and attribute names & types. **Provided for each intended use of the Wallet-Relying Party.**   |  WalletRelyingParty.WalletRelyingPartyService.IntendedUse.credentials    |
...
The table contains clarifications regarding use of the data attributes when 1) the Wallet-Relying Parties are registering attestation provider entitlement(s), or 2) are as service providers relying on the option of using an intermediary (a Wallet-Relying Party Service registered to operate as an intermediary) for transactions with the EUDI Wallet Units.
...
| Use of Intermediary         | List of intermediaries the Wallet-Relying Party Service with entitlement type 'Service_provider' relies upon SHALL be provided, if it is not transacting directly with the Wallet Units. When present, the intermediary-specific data object SHALL contain its unique identifier and its Trade Name.   | WalletRelyingParty.WalletRelyingPartyService.usesIntermediary  |
| Is Intermediary             | An indicator flag that SHALL be present if the registered Wallet-Relying Party Service intends to act as an intermediary for other Wallet-Relying Parties. *Note: If the same Wallet-Relying Party provides a service which requires presentation of attestations, it SHALL register this service as a separate registration instance, with appropriate data on its intended use(s).*    | WalletRelyingParty.WalletRelyingPartyService.isIntermediary  |
| Provides Attestations       | A list of attestation types the Wallet-Relying Party Service intends to issue for Wallet Units. SHALL only be present if the entitlements of the Wallet-Relying Party Service are of type 'QEAA_Provider', 'Non_Q_EAA_Provider', 'PUB_EAA_Provider' or 'PID_Provider'. | WalletRelyingParty.WalletRelyingPartyService.providesAttestations    |
